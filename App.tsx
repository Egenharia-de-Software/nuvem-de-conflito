...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const storedExpandedId = localStorage.getItem('expandedCloudId');
    if (storedExpandedId && initialClouds.some(c => c.id === storedExpandedId)) {
        lastExpandedId = storedExpandedId;
    } else if (initialClouds.length > 0) {
        const firstUnsolved = initialClouds.find(c => !c.isSolved);
        lastExpandedId = firstUnsolved ? firstUnsolved.id : initialClouds[0].id;
    }
    setExpandedCloudId(lastExpandedId);
    setIsLoading(false);
  }, []);
  ...
  useEffect(() => {
    if (!isLoading) { 
        if (expandedCloudId) {
          localStorage.setItem('expandedCloudId', expandedCloudId);
        } else {
          localStorage.removeItem('expandedCloudId');
        }
    }
  }, [expandedCloudId, isLoading]);
  ...
  const handleCreateCloud = () => {
    ...
    setExpandedCloudId(newCloud.id); 
  };
  ...
  const handleDeleteCloud = (cloudId: string) => {
    setConflictClouds((prevClouds) => {
      ...
      if (expandedCloudId === cloudId) {
        setExpandedCloudId(remainingClouds.length > 0 ? remainingClouds[0].id : null);
      }
      return remainingClouds;
    });
  };

  const handleToggleExpandCloud = (cloudId: string) => {
    setExpandedCloudId((prevExpandedId) => (prevExpandedId === cloudId ? null : cloudId));
  };
    ...
    <div className="min-h-screen flex flex-col bg-slate-100">
      ...
      <main className="flex-grow container mx-auto p-3 sm:p-4 grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6 items-start">
            ...
            <ul className="space-y-1.5 mt-3">
              {conflictClouds.map((cloud) => (
                <CloudListItem
                  key={cloud.id}
                  cloud={cloud}
                  isExpanded={expandedCloudId === cloud.id}
                  onToggleExpand={() => handleToggleExpandCloud(cloud.id)}
        ...
        <section className="lg:col-span-8 xl:col-span-9 min-h-[300px]">
          {expandedCloudId === null && conflictClouds.length > 0 && (
           ...
           {expandedCloudId !== null && conflictClouds.find(c => c.id === expandedCloudId) === undefined && (
