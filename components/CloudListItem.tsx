...
import ConflictCloudEditor from './ConflictCloudEditor'; 
...
const ChevronRightIcon: React.FC<{ className?: string }> = ({ className }) => ( // Changed to ChevronRight for collapsed state
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={`w-5 h-5 ${className}`}>
...
const CloudListItem: React.FC<CloudListItemProps> = ({
  ...
  const handleHeaderClick = (e: React.MouseEvent<HTMLDivElement> | React.KeyboardEvent<HTMLDivElement>) => {
    if (e.target instanceof HTMLElement && e.target.closest('button[aria-label^="Excluir nuvem"]')) {
        return;
    }
    onToggleExpand();
  };
  ...
  return (
    <li className={itemContainerClasses}>
      <div
        ...
        role="button"
        tabIndex={0}
        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleHeaderClick(e);}}
        aria-expanded={isExpanded}
        aria-controls={`cloud-editor-${cloud.id}`}
      >
        <div className="flex items-center gap-2 overflow-hidden flex-grow min-w-0">
          {cloud.isSolved && (
            <CheckCircleIcon className={`w-4 h-4 flex-shrink-0 ${isExpanded ? 'text-sky-600' : 'text-emerald-500'}`} />
          )}
          {!cloud.isSolved && !isExpanded && (
             <span className={`w-4 h-4 flex-shrink-0 text-slate-400`}> {/* Placeholder for alignment or future icon */}
           ...
           {!cloud.isSolved && isExpanded && (
             <span className={`w-4 h-4 flex-shrink-0 text-sky-600`}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                 <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25" />
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.75 9.75h4.5m-4.5 3h4.5m-4.5 3h4.5M4.5 19.5h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" />
                </svg>
          ...
          <span className={`font-medium truncate text-xs ${isExpanded ? 'text-sky-700' : (cloud.isSolved ? 'text-emerald-700' : 'text-slate-700')}`} title={cloud.title}>
            {cloud.title || 'Nuvem Sem Título'}
          </span>
        </div>
        <div className="flex items-center flex-shrink-0 pl-2">
            {isExpanded ? <ChevronDownIcon className={`w-4 h-4 ${isExpanded ? 'text-sky-600' : 'text-slate-500' }`} /> : <ChevronRightIcon className={`w-4 h-4 ${cloud.isSolved ? 'text-emerald-500' : 'text-slate-500'}`} />}
        </div>
      </div>
      {isExpanded && (
        <div 
            id={`cloud-editor-${cloud.id}`} 
            role="region" 
            className="bg-white p-0" // Padding handled by ConflictCloudEditor
        >
          <ConflictCloudEditor
            cloud={cloud}
            onUpdateCloud={onUpdateCloud}
